...
public class DisjunctiveNormalForm extends NormalForm {
    ...
    public Expr normalize() {
        List<Expr> orList = new ArrayList<>();
        for (VarAssignment it : getTrueAssignments()) {
            List<Expr> andList = new ArrayList<>();
            for (Var var : it.getVars()) {
            ...
            if (andList.size() >= 2){
                AndExpr andRes = new AndExpr(andList.remove(0), andList.remove(0));
            ...
            if (orList.size() >= 2){
                OrExpr orRes = new OrExpr(orList.remove(0), orList.remove(0));
